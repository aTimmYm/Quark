...
function TabBar:onMouseUp(btn, x, y)
	...
	if not self:check(x, y) or not click then return end
	if click.close or btn == 3 then
		...
		elseif click.index == self.selected then
			...
			if self.pressed then self:pressed(self.selected, btn, x, y) end
		end
	elseif self.selected ~= click.index then
		if btn == 1 then self.selected = click.index end
		if self.pressed then self:pressed(click.index, btn, x, y) end
	end
...
function TabBar:onMouseDown(btn, x, y)
	...
	local index = math.ceil(l_x / self.max_w)
	if index > #self.tabs then return true end
	self.click = { index = index, close = l_x % self.max_w == 0 }
