...
local onCommand = {
	['Find'] = function(editor)
		...
		function search_panel:onSelectResult(sX, eX, sY, query, case_sensitivity, whole_words, regular_expressions)
			...
			if sX and sY then
				c_editor:setCursor(cursor, sX, sY)
			end
	...
	['Save'] = function(editor)
		local ok, err = editor.document:save()
		if not ok then
			-- СПРОСИТЬ ПУТЬ (короче окно)
		end
	end
...
function root.custom_handlers.key(key)
	if key == keys.leftShift or key == keys.rightShift then
		root.SHIFT_HELD = true
		return true
...
function root.custom_handlers.key_up(key)
	if key == keys.leftShift or key == keys.rightShift then
		root.SHIFT_HELD = false; return true
	elseif key == keys.leftAlt or key == keys.rightAlt then
		root.ALT_HELD = false; return true
	elseif key == keys.leftCtrl or key == keys.rightCtrl then
		root.CTRL_HELD = false; return true
	end
end
...
function root:onTerminate()
	...
	if root.workspace and sidebar.path then
		...
		if file then
			...
			file:write('return ' .. textutils.serialise(project))
			file:close()
		end
	end
	user()
...
if vfs.exists(APPDIR .. '/Data/projects/project.lua') then
	...
	tabbar:pressed(tabbar.selected)
else
