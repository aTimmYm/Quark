...
function ColorSelector:draw()
	...
	if color_index and self.items[color_index] then
		...
		elseif current_color ~= to_hex[7] then
...
function ColorSelector:setPossibleColors(table)
	self.items = expect(table, 'table', 'table', 'nil') or default_palette
end
...
function ColorSelector.new(args)
	...
	instance.items = expect_args(args, 'items', 'table', 'nil') or default_palette
