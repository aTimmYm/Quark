...
local function editor_onMouseScroll(self, dir, x, y)
	if self.alt_held then dir = dir * 5 end
...
local function editor_onMouseDown(self, btn, x, y)
	...
	if x - self.x + 1 < 6 then
		...
		if not self.shift_held then current_cursor.cx, current_cursor.cy = 1, p_y end
		cursors.panel = true
	...
	if cursors.panel and self.shift_held then
	...
	if self.shift_held and not self.alt_held then
		self:selectText(current_cursor, current_cursor.cx, current_cursor.cy, current_cursor.x, current_cursor.y)
	elseif not self.alt_held then
...
local function editor_onKeyDown(self, key, held)
	...
	elseif key == keys.left then
		...
		for id = 1, #cursors do
			...
			if cursor.x > 1 or cursor.y > 1 then
				if selected and not self.shift_held then
	...
	elseif key == keys.up then
		...
		if self.alt_held and self.ctrl_held then
			local total = #cursors
	...
	elseif key == keys.s then
		...
		for n = self:getLinesSize(), 1, -1 do
			if not self:getLine(n):match('%S') then
				self:setLine(n, nil)
			else
				break
			end
		...
		self:setLine(self:getLinesSize() + 1, '')
